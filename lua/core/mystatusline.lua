...
local function get_lsp_status()
  ...
  for _, client in ipairs(clients) do
    -- Get the filetypes supported by the client
    ---@diagnostic disable-next-line: undefined-field
    local filetypes = client.config.filetypes
    -- Check if the current filetype is supported by the client
    if filetypes and vim.fn.index(filetypes, vim.bo.filetype) ~= -1 then
      -- Return the LSP status string
      return string.format("%%#StatusLineLSP#[%s]", client.name)
    end
  end
  -- Return an empty string if no LSP client is found for the current filetype
  return ""
end
...
local model_patterns = {
  {
    ...
    handle = function(model)
      ...
      return variant
          and variant:gsub("(%w+)", function(h)
            if model == "claude-3-5-haiku" then
              return "haiku3.5"
            elseif model == "claude-3-haiku" then
              return "haiku3"
            elseif model:match("claude%-3%-5%-(%w+)") then
              return h .. "3.5"
            elseif model:match("claude%-3%-(%w+)") then
              return h .. "3"
            end
          end)
        or model
...
-- Thanks to https://github.com/nvzone/timerly/discussions/4
local function get_timerly_status()
  if package.loaded["timerly"] == nil then
    return ""
  end
  local state = require("timerly.state")
  if state.progress == 0 or state.status == "pause" then
    return ""
  end
