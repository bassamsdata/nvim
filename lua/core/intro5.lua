local M = {}
local actions = {
	["NVIMRC"] = "e ~/.config/nvim/init.lua",
	["FIND-FILES"] = "Pick files",
	["OLDFILES"] = "Pick oldfiles",
	["LAST-SESSION"] = "echo 'Nothing!'",
}
local hls = {
	Error = "[░▒]",
	Function = "[▓█▄▀▐▌]",
...
M.setup = vim.schedule_wrap(function()
	...
	if arg and (vim.fn.isdirectory(arg) ~= 0) or arg == "" then
		Do = function(word)
			vim.cmd(actions[word] or 'echo "Aaaaah!"')
		end
		...
		vim.keymap.set(
			"n",
			"<LeftMouse>",
			'<LeftMouse><Cmd>lua Do(vim.fn.expand("<cword>"))<CR>',
			{ buffer = 0 }
		)
		...
		vim.api.nvim_create_autocmd("BufLeave", {
			once = true,
			callback = function()
				vim.iter(items):map(function(id)
