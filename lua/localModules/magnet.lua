...
local function selectSymbol(symbols)
  ...
  local longestName = math.max(unpack(vim.tbl_map(function(line)
    return #line
  end, names)))
  ...
  for i = 1, #symbols do
    ...
    if key ~= "" then
      ...
      vim.api.nvim_buf_add_highlight(
        bufnr,
        ns,
        config.hints.highlight,
        lnum,
        col - 1,
        col
      )
      vim.keymap.set("n", key, function()
        jumpToSymbol(symbols[i], winnr)
      end, opts)
...
local function filterSymbols(symbols)
  ...
  local excludeResults = config.excludeResults[vim.bo.filetype]
    or config.excludeResults.default

  local filteredSymbols = vim
    .iter(symbols)
    :map(function(symbol)
